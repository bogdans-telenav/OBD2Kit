...
- (NSStreamStatus)status {
    if (_inputStream && _outputStream) {
        if (_inputStream.streamStatus == NSStreamStatusNotOpen ||  _outputStream.streamStatus == NSStreamStatusNotOpen) {
            return NSStreamStatusNotOpen;
        }
        
        if (_inputStream.streamStatus == NSStreamStatusOpening ||  _outputStream.streamStatus == NSStreamStatusOpening) {           
        ...
        if (_inputStream.streamStatus == NSStreamStatusClosed ||  _outputStream.streamStatus == NSStreamStatusClosed) {
            return NSStreamStatusClosed;
        }
        
        if (_inputStream.streamStatus == NSStreamStatusReading) {
            return NSStreamStatusReading;
        }
        
        if (_outputStream.streamStatus == NSStreamStatusWriting) {
            return NSStreamStatusWriting;
        }
        
        if (_inputStream.streamStatus == NSStreamStatusAtEnd || _outputStream.streamStatus == NSStreamStatusAtEnd) {
...
- (NSError *)error {
    if (_inputStream.streamError) {
        ...
        return _outputStream.streamError;
    }
}
...
//        }
		...
		FLDEBUG(@"_readBufLength = %ld", (unsigned long)_readBufLength)
...
- (void)handleInputEvent:(NSStreamEvent)eventCode
{
	if(_inputStream) {
		switch (eventCode) {
			...
			default:
                FLINFO(@"ORICE ALTCEVA");
				break;
		}
		
    } else {
        FLINFO(@"NU MAI AVEM INPUT");
    }
