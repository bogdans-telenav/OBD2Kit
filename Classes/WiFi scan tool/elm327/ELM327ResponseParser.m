...
- (NSArray*) parseResponse:(FLScanToolProtocol)protocol {
	...
	if(!ELM_ERROR(asciistr) /*&& ELM_DATA_RESPONSE(asciistr)*/) {
        ...
        NSString *response = [self stringForResponse];
        NSString *striptedResp = [response stringByReplacingOccurrencesOfString:@"\r\n" withString:@""];
		NSArray	*responseComponents = [striptedResp componentsSeparatedByString:@"\r"];
