...
+ (ELM327Command*) commandForOBD2:(FLScanToolMode)mode pid:(NSUInteger)pid data:(NSData*)data {
	...
	if (pid <= OBD2SensorLast)
		cmd = [[self alloc] initWithCommandString:[NSString stringWithFormat:@"%02lx %02lx", (unsigned long)mode, (unsigned long)pid]];
	else
		cmd = [[self alloc] initWithCommandString:[NSString stringWithFormat:@"%02lx", (unsigned long)mode]];
...
+ (ELM327Command*) commandForReset {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327Reset];
	return cmd;
}

+ (ELM327Command*) commandForHeadersOn {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327HeadersOn];
	return cmd;
}


+ (ELM327Command*) commandForEchoOff {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327EchoOff];
	return cmd;
}


+ (ELM327Command*) commandForReadVoltage {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327ReadVoltage];
	return cmd;
}


+ (ELM327Command*) commandForReadProtocol {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327ReadProtocolNumber];
	return cmd;
}


+ (ELM327Command*) commandForReadVersionID {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327ReadVersionID];
	return cmd;
}


+ (ELM327Command*) commandForReadDeviceDescription {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327ReadDeviceDescription];
	return cmd;
}


+ (ELM327Command*) commandForReadDeviceIdentifier {
	ELM327Command* cmd = [[self alloc] initWithCommandString:kELM327ReadDeviceIdentifier];
	return cmd;
}


+ (ELM327Command*) commandForSetDeviceIdentifier:(NSString*)identifier {
	ELM327Command* cmd = [[self alloc] initWithCommandString:[NSString stringWithFormat:@"%@ %@", kELM327SetDeviceIdentifier, identifier]];
	return cmd;
}
